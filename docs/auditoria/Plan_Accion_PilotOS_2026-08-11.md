...
- [x] **G1.1 (2026-08-11) — HECHO.** Volvió a pasar el 11 de agosto (segunda vez en cuatro días), así que se cerró la causa en vez del síntoma. `POST /recuperar` + `POST /restablecer`, código de 6 dígitos por WhatsApp vía GlorIA, hash del código en `PasswordReset`, un solo uso, 15 min, 5 intentos, respuesta neutra para no permitir enumerar usuarios, cambio+quemado en la misma transacción. Frontend con «He olvidado mi contraseña». 13 tests nuevos. Ver C-053. **Falta la plantilla `codigo_recuperacion` en Meta** para que el código llegue.
...
- [x] **G3.2-terreno (2026-08-11)** — el tramo de recepción/guardado ya no depende de G2.1: en vez de generalizar `DocumentoEnlace` (arriesgado, ~10 sitios lo consultan), se añadieron `vehiculo_id`/`mantenimiento_vehiculo_id` directamente a `Documento` (aditivo) y `POST /internal/documentos-vehiculo`. Ver C-048.
- [x] **G3.2-cable (2026-08-11)** — **la foto ya entra al sistema.** Causa raíz que no era la que parecía: no fallaba la clasificación, es que el fichero nunca llegaba — Meta manda un `id`, no la imagen, y el bridge de n8n no lo copiaba. Construido: `MetaMedia.ts` (descarga de Meta), `pilotos.client.ts` (entrega a PilotOS), gancho en `inbound.routes.ts`, y `documentos-vehiculo` resolviendo el vehículo por teléfono. Cambio en n8n mínimo (2 nodos de 19, conexiones intactas). Ver C-052.
- [ ] **G3.2-extracción** — lo único que queda: clasificar el documento (factura de taller / ITV / seguro) y encajarlo con el mantenimiento que resuelve. El documento entra hoy como `DOCUMENTO_VEHICULO_SIN_CLASIFICAR` y ahí se queda. **Espera a ver una foto real** — misma prudencia que obligó a rehacer el parser del ticket (C-043): las reglas de OCR escritas a ciegas fallan contra el documento de verdad.
