...
## DT-037 · Intro del hero con logo grande → cabecera (patrón compartido con RentOS)

- Fecha: 2026-08-18
- Area: Frontend / landing (`app/src/app/page.tsx`, `app/src/components/marketing/hero-intro.tsx`, `app/src/app/globals.css`)
- Decision: Al cargar la landing, un logo grande se muestra centrado sobre el hero, se queda quieto un momento y se encoge con la Web Animations API hasta ocupar exactamente el hueco del logo de la cabecera (posición y tamaño medidos en tiempo real con `getBoundingClientRect`, no valores fijos). Solo cuando esa animación termina se revela el resto del hero: la foto entra deslizándose desde la izquierda y el texto + las tarjetas aparecen en cascada. Un `<script>` inline añade la clase `pilotos-intro` al `<html>` antes del primer pintado, para que el CSS pueda ocultar el estado final (logo de cabecera, foto, texto, tarjetas) sin parpadeo mientras la intro corre.
- Alternativa descartada: mantener las animaciones de entrada disparándose todas a la vez al montar (lo que había antes) — se veía bien pero no comunicaba "esto es PilotOS" con la fuerza de un logo protagonista, y no daba pie a que la foto entrara con sensación de movimiento.
- Justificacion: **este patrón ya existía en RentOS** (`RentOS/nexos-api/frontend/index.html`, función `heroIntro()`) y funcionaba bien. Se replicó tal cual en vez de inventar uno nuevo — la regla de arquitectura NexOS de reutilizar antes que duplicar aplica también a un patrón de UI/animación, no solo a lógica de negocio. Si otro producto OS quiere el mismo efecto de intro en su landing, la referencia es este componente (`hero-intro.tsx`) o el de RentOS, no reinventarlo por tercera vez.
- Impacto: Respeta `prefers-reduced-motion` (salta directo al estado final, sin logo grande) y tiene una red de seguridad por `setTimeout` de 6s por si `getBoundingClientRect` no encuentra medidas válidas (imagen que no llega a cargar, etc.) — nunca se queda la landing sin logo de cabecera ni contenido visible.
- Riesgo aceptado: sin JavaScript, el hero se queda oculto (mismo trade-off que RentOS) — es una landing moderna que ya asume JS para el resto de interacciones, no una degradación aceptable para un sitio que necesite funcionar sin JS.
