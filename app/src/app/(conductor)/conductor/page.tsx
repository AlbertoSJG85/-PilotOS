...
function EstadoIcon({ estado }: { estado: string }) {
  ...
  return <AlertCircle className="h-4 w-4 text-zinc-500 shrink-0" />;
}
...
export default function ConductorHome() {
  ...
  // Parte de hoy usando hora LOCAL del dispositivo (no UTC)
  const today = localDateString();
  const parteHoy = partes.find((p) => {
    // fecha_trabajada viene como "2026-04-07T00:00:00.000Z" o "2026-04-07"
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-lg mx-auto">
      ...
      {/* Top bar — pt-safe-top incluye notch de iPhone */}
      <header className="flex items-center justify-between px-5 pt-safe-top pb-4 border-b border-zinc-800/60">
        <div>
          <h1 className="text-lg font-bold text-zinc-100">
            Pilot<span className="text-amber-500">OS</span>
          </h1>
          <p className="text-xs text-zinc-500 truncate max-w-[180px]">{user?.nombre || 'Conductor'}</p>
        </div>
        <button
      ...
      <main className="flex-1 px-5 py-6 space-y-5">
        ...
        {/* Vehículo — con skeleton durante carga */}
        {loading ? (
          ...
          <div className="rounded-2xl bg-zinc-900 border border-zinc-800 px-5 py-4">
            ...
            <p className="text-xl font-bold text-zinc-100 tracking-wider">{vehiculo.matricula}</p>
        ...
        {!loading && (
          parteHoy ? (
            ...
            <Link
              ...
              className="block w-full rounded-2xl bg-amber-500 hover:bg-amber-400 active:bg-amber-600 active:scale-[0.98] transition-all py-8 text-center shadow-lg shadow-amber-500/20"
            >
        ...
        {/* Loader del botón mientras carga */}
        {loading && (
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-zinc-800/60 bg-zinc-900/40 py-10 text-center">
            ...
            <div className="space-y-2">
              {partes.map((p) => (
                <Link
                  ...
                  className="flex items-center justify-between rounded-xl border border-zinc-800/60 bg-zinc-900/60 px-4 py-3 active:bg-zinc-800 transition-colors"
                >
